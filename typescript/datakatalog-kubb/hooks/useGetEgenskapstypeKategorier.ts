/**
 * Generated by Kubb (https://kubb.dev/).
 * Do not edit manually.
 */

import fetch from '@kubb/plugin-client/clients/axios'
import type {
  GetEgenskapstypeKategorierQueryResponse,
  GetEgenskapstypeKategorier404,
  GetEgenskapstypeKategorier500,
  GetEgenskapstypeKategorier503,
} from '../models/GetEgenskapstypeKategorier.ts'
import type {
  RequestConfig,
  ResponseErrorConfig,
} from '@kubb/plugin-client/clients/axios'
import type {
  QueryKey,
  QueryClient,
  QueryObserverOptions,
  UseQueryResult,
} from '@tanstack/react-query'
import { getEgenskapstypeKategorierQueryResponseSchema } from '../schemas/getEgenskapstypeKategorierSchema.ts'
import { queryOptions, useQuery } from '@tanstack/react-query'

export const getEgenskapstypeKategorierQueryKey = () =>
  [{ url: '/api/v1/egenskapstypekategorier' }] as const

export type GetEgenskapstypeKategorierQueryKey = ReturnType<
  typeof getEgenskapstypeKategorierQueryKey
>

/**
 * @summary Returnerer alle kategorier for egenskapstypene
 * {@link /api/v1/egenskapstypekategorier}
 */
export async function getEgenskapstypeKategorier(
  config: Partial<RequestConfig> & { client?: typeof fetch } = {},
) {
  const { client: request = fetch, ...requestConfig } = config

  const res = await request<
    GetEgenskapstypeKategorierQueryResponse,
    ResponseErrorConfig<
      | GetEgenskapstypeKategorier404
      | GetEgenskapstypeKategorier500
      | GetEgenskapstypeKategorier503
    >,
    unknown
  >({ method: 'GET', url: `/api/v1/egenskapstypekategorier`, ...requestConfig })
  return getEgenskapstypeKategorierQueryResponseSchema.parse(res.data)
}

export function getEgenskapstypeKategorierQueryOptions(
  config: Partial<RequestConfig> & { client?: typeof fetch } = {},
) {
  const queryKey = getEgenskapstypeKategorierQueryKey()
  return queryOptions<
    GetEgenskapstypeKategorierQueryResponse,
    ResponseErrorConfig<
      | GetEgenskapstypeKategorier404
      | GetEgenskapstypeKategorier500
      | GetEgenskapstypeKategorier503
    >,
    GetEgenskapstypeKategorierQueryResponse,
    typeof queryKey
  >({
    queryKey,
    queryFn: async ({ signal }) => {
      config.signal = signal
      return getEgenskapstypeKategorier(config)
    },
  })
}

/**
 * @summary Returnerer alle kategorier for egenskapstypene
 * {@link /api/v1/egenskapstypekategorier}
 */
export function useGetEgenskapstypeKategorier<
  TData = GetEgenskapstypeKategorierQueryResponse,
  TQueryData = GetEgenskapstypeKategorierQueryResponse,
  TQueryKey extends QueryKey = GetEgenskapstypeKategorierQueryKey,
>(
  options: {
    query?: Partial<
      QueryObserverOptions<
        GetEgenskapstypeKategorierQueryResponse,
        ResponseErrorConfig<
          | GetEgenskapstypeKategorier404
          | GetEgenskapstypeKategorier500
          | GetEgenskapstypeKategorier503
        >,
        TData,
        TQueryData,
        TQueryKey
      >
    > & { client?: QueryClient }
    client?: Partial<RequestConfig> & { client?: typeof fetch }
  } = {},
) {
  const { query: queryConfig = {}, client: config = {} } = options ?? {}
  const { client: queryClient, ...queryOptions } = queryConfig
  const queryKey =
    queryOptions?.queryKey ?? getEgenskapstypeKategorierQueryKey()

  const query = useQuery(
    {
      ...getEgenskapstypeKategorierQueryOptions(config),
      queryKey,
      ...queryOptions,
    } as unknown as QueryObserverOptions,
    queryClient,
  ) as UseQueryResult<
    TData,
    ResponseErrorConfig<
      | GetEgenskapstypeKategorier404
      | GetEgenskapstypeKategorier500
      | GetEgenskapstypeKategorier503
    >
  > & { queryKey: TQueryKey }

  query.queryKey = queryKey as TQueryKey

  return query
}
